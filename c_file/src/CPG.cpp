...
void CPG::getXdot(double t, double *x, double *xdot,double Iext,double **Xprec)
{
     ...
     * double Vjprec;
     *
     * int maxNdelay = this->NnetwDelays;
     * int *delays_;
     * double **Xprec_;
     *
     ...
     * }
     *
     * inhActivation[matrInd]->getXdot(t,x+Vindex[N+matrInd],xdot+Vindex[N+matrInd]);
     *
     * if(g_ex[matrInd] != 0)
     * {
     ...
     * }
     *
     * excActivation[matrInd]->getXdot(t,x+Vindex[N+N*N+matrInd],xdot+Vindex[N+N*N+matrInd]);
     *
     * if(g_el[matrInd] != 0)
     ...
     * {
     * Xprec_[k] = Xprec[neuronDelaysIndex[matrInd]->at(k)+Vindex[N+matrInd]];
     * }
     *
     * (neuroni[i])->getXdot(t,x+Vindex[i],xdot+Vindex[i],Isyn,Xprec);
     * }
     *
     *
     * //free(Vprec);
...
/*bool CPG::getResetConditions(double *x,double **Xprec)
{
    int N = this->N;
    int i,j,k;
    neuron_model **neuroni = this->neuroni;
    ...
    Vjinprec = (double *)malloc(maxNdelay*sizeof(double));
    Vjexprec = (double *)malloc(maxNdelay*sizeof(double));
    ...
    for(i=0;i<N*N;i++)
    {
        ...
        for(k=0;k<NinhSynDelaysIndex[i];k++)
        {
            Vjinprec[k] = Xprec[inhSynDelaysIndex[i]->at(k)][Vindex[j]];
        }
        
        for(k=0;k<NexcSynDelaysIndex[i];k++)
        {
            Vjexprec[k] = Xprec[excSynDelaysIndex[i]->at(k)][Vindex[j]];
        }
        
        
        if(((inhActivation[i])->getResetConditions(x+Vindex[N+i],Vj,Vjinprec)) || ((excActivation[i])->getResetConditions(x+Vindex[N+N*N+i],Vj,Vjexprec)))
        {
            return true;
            break;
        }
...
/*void CPG::resetStates(double *x,double **Xprec)
{
    int N = this->N;
    int i,j,k;
    neuron_model **neuroni = this->neuroni;
    ...
    for(i=0;i<N*N;i++)
    {
        ...
        if((inhActivation[i])->getResetConditions(x+Vindex[N+i],Vj,Vjinprec))
        {
            (inhActivation[i])->resetStates(x+Vindex[N+i],Vj,Vjexprec);
        }
        
        if((excActivation[i])->getResetConditions(x+Vindex[N+N*N+i],Vj,Vjexprec))
        {
            (excActivation[i])->resetStates(x+Vindex[N+N*N+i],Vj,Vjexprec);
        }
    }
