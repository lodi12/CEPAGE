...
parfor i=1:nDeltaPhi
    for k=1:nStepIntegral
        ...
        w_ex_(i) = w_ex_(i)+Q*(-V1+EsynEx)*excActivation(Vj);
        
        w_in_(i) = w_in_(i)+ Q*(-V1+EsynIn)*inhActivation(Vj);
...
for i=1:nDeltaPhi
    parfor j=1:nDeltaPhi
        for k=1:nStepIntegral
            ...
            M_ex_(i,j) = M_ex_(i,j)+Q*(-Vi+EsynEx)/(1+exp(-uSyn*(Vj-tetaSyn)));
            
            M_in_(i,j) = M_in_(i,j)+ Q*(-Vi+EsynIn)/(1+exp(-uSyn*(Vj-tetaSyn)));
